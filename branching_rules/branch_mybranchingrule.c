 ...
 /**@file   branch_mybrancingrule.c
  * @ingroup BRANCHINGRULES
  * @brief  probability based branching rule based on an article by J. Pryor and J.W. Chinneck
  * @author Gregor Hendel
  *
  * The mybrancingrule branching rule selects a variable based on its impact on row activity mybrancingrules. More formally,
  ...
  * With these two parameters, we can calculate the probability to satisfy the row in terms of the cumulative mybrancingrule
  * of the standard normal mybrancingrule: \f$ P(a(x) \leq b) = \Phi(\frac{b - \mu}{\sigma})\f$.
  *
 ...
 #include "scip/branch_mybrancingrule.h"
 ...
 #define BRANCHRULE_NAME            "mybrancingrule"
 #define BRANCHRULE_DESC            "branching rule based on variable influence on cumulative normal mybrancingrule of row activities"
 ...
 #define EVENTHDLR_NAME             "eventhdlr_mybrancingrule"    /**< event handler name */
 #define EVENT_mybrancingrule         SCIP_EVENTTYPE_BOUNDCHANGED /**< the event tyoe to be handled by this event handler */
 ...
 {
    SCIP_BRANCHRULEDATA*  branchruledata;     /**< the branching rule data to access mybrancingrule arrays */
 };
    ...
    )
 {
    ...
    if( branchruledata->memsize == 0 )
    {
       ...
       for( v = 0; v < nvars; ++v )
       {
          ...
          SCIP_CALL( SCIPcatchVarEvent(scip, vars[v], EVENT_mybrancingrule, branchruledata->eventhdlr, NULL, &(branchruledata->varfilterposs[v])) );
 ...
 /** calculate the variable's mybrancingrule parameters (mean and variance) for the bounds specified in the arguments.
  *  special treatment of infinite bounds necessary */
 void SCIPvarCalcmybrancingruleParameters(
    SCIP*                 scip,               /**< SCIP data structure */
    ...
    SCIP_Real*            variance            /**< pointer to store the variance of the variable uniform mybrancingrule */
    )
 {
    assert(mean != NULL);
    assert(variance != NULL);
 
    /* calculate mean and variance of variable uniform mybrancingrule before and after branching */
    ...
    {
       /* if the variable is continuous, we assume a continuous uniform mybrancingrule, otherwise a discrete one */
 ...
 /** calculates the cumulative mybrancingrule P(-infinity <= x <= value) that a normally distributed
  *  random variable x takes a value between -infinity and parameter \p value.
  *
  *  The mybrancingrule is given by the respective mean and deviation. This implementation
  *  uses the error function SCIPerf().
  */
 SCIP_Real SCIPcalcCumulativemybrancingrule(
    SCIP*                 scip,               /**< current SCIP */
    SCIP_Real             mean,               /**< the mean value of the mybrancingrule */
    SCIP_Real             variance,           /**< the square of the deviation of the mybrancingrule */
    SCIP_Real             value               /**< the upper limit of the calculated mybrancingrule integral */
    )
 {
    SCIP_Real normvalue;
    SCIP_Real std;
    ...
    /* scale and translate to standard normal mybrancingrule. Factor sqrt(2) is needed for SCIPerf() function */
    normvalue = (value - mean)/(std * SQRTOFTWO);
 
    SCIPdebugMsg(scip, " Normalized value %g = ( %g - %g ) / (%g * 1.4142136)\n", normvalue, value, mean, std);
 
    /* calculate the cumulative mybrancingrule function for normvalue. For negative normvalues, we negate
  ...
  *
  *  For inequalities, we use the cumulative mybrancingrule function of the standard normal
  *  mybrancingrule PHI(rhs - mu/sqrt(sigma2)) to calculate the probability
 ...
 SCIP_Real SCIProwCalcProbability(
    ...
    SCIP_Real             mu,                 /**< the mean value of the row mybrancingrule */
    SCIP_Real             sigma2,             /**< the variance of the row mybrancingrule */
    ...
    )
 {
    ...
    /* use the cumulative mybrancingrule if the row contains no variable to repair every infeasibility */
    if( !SCIPisInfinity(scip, rhs) && rowinfinitiesdown == 0 )
       rhsprob = SCIPcalcCumulativemybrancingrule(scip, mu, sigma2, rhs);
    ...
    if( !SCIPisInfinity(scip, -lhs) && rowinfinitiesup == 0 )
       lhsprob = 1.0 - SCIPcalcCumulativemybrancingrule(scip, mu, sigma2, lhs);
 ...
 /** calculates the initial mean and variance of the row activity normal mybrancingrule.
  *
 ...
 void rowCalculateGauss(
    ...
    SCIP_ROW*             row,                /**< the row for which the gaussian normal mybrancingrule has to be calculated */
    SCIP_Real*            mu,                 /**< pointer to store the mean value of the gaussian normal mybrancingrule */
    SCIP_Real*            sigma2,             /**< pointer to store the variance value of the gaussian normal mybrancingrule */
    ...
    )
 {
    ...
    *rowinfinitiesup = 0;
    ...
    for( c = 0; c < nrowvals; ++c )
    {
       ...
       SCIPvarCalcmybrancingruleParameters(scip, colvarlb, colvarub, SCIPvarGetType(colvar), &varmean, &varvariance);
  ...
  */
 SCIP_RETCODE SCIPupdatemybrancingruleScore(
    ...
    )
 {
    ...
    SCIP_Real currentmean;      /* current mean value of variable uniform mybrancingrule */
    SCIP_Real meanup;           /* mean value of variable uniform mybrancingrule after branching up */
    SCIP_Real meandown;         /* mean value of variable uniform mybrancingrule after branching down*/
    ...
    SCIPvarCalcmybrancingruleParameters(scip, varlb, varub, vartype, &currentmean, &squaredbounddiff);
 
    newlb = SCIPfeasCeil(scip, lpsolval);
    newub = SCIPfeasFloor(scip, lpsolval);
 
    /* calculate the variable's uniform mybrancingrule after branching up and down, respectively. */
    squaredbounddiffup = 0.0;
    meanup = 0.0;
    SCIPvarCalcmybrancingruleParameters(scip, newlb, varub, vartype, &meanup, &squaredbounddiffup);
 
    /* calculate the mybrancingrule mean and variance for a variable with finite lower bound */
    squaredbounddiffdown = 0.0;
    meandown = 0.0;
    SCIPvarCalcmybrancingruleParameters(scip, varlb, newub, vartype, &meandown, &squaredbounddiffdown);
    ...
    *downscore = 0.0;
    ...
    for( i = 0; i < ncolrows; ++i )
    {
       ...
       /* calculate row activity mybrancingrule if this is the first candidate to appear in this row */
       ...
       /* retrieve the row mybrancingrule parameters from the branch rule data */
       ...
       SCIP_CALL( SCIPupdatemybrancingruleScore(scip, currentrowprob, newrowprobup, newrowprobdown, upscore, downscore, scoreparam) );
    ...
    )
 {
    ...
    /* calculate old and new variable mybrancingrule mean and variance */
    oldvariance = 0.0;
    newvariance = 0.0;
    oldmean = 0.0;
    newmean = 0.0;
    vartype = SCIPvarGetType(var);
    SCIPvarCalcmybrancingruleParameters(scip, oldlb, oldub, vartype, &oldmean, &oldvariance);
    SCIPvarCalcmybrancingruleParameters(scip, newlb, newub, vartype, &newmean, &newvariance);
 
    /* loop over all rows of this variable and update activity mybrancingrule */
    for( r = 0; r < ncolrows; ++r )
    {
       ...
       /* only consider rows for which activity mybrancingrule was already calculated */
       if( branchruledata->rowmeans[rowpos] != SCIP_INVALID ) /*lint !e777*/
       {
          SCIP_Real coeff;
          ...
          /* update variable contribution to row activity mybrancingrule */
 ...
 static
 SCIP_DECL_EVENTFREE(eventFreemybrancingrule)
 {
 ...
 static
 SCIP_DECL_BRANCHCOPY(branchCopymybrancingrule)
 {  /*lint --e{715}*/
    assert(scip != NULL);
 
    SCIP_CALL( SCIPincludeBranchrulemybrancingrule(scip) );
 ...
 static
 SCIP_DECL_BRANCHEXITSOL(branchExitsolmybrancingrule)
 {
    ...
    if( branchruledata->varfilterposs != NULL)
    {
       ...
       for( v = 0; v < nvars; ++v )
       {
          SCIP_CALL( SCIPdropVarEvent(scip, vars[v], EVENT_mybrancingrule, branchruledata->eventhdlr, NULL, branchruledata->varfilterposs[v]) );
       }
 ...
 static
 SCIP_DECL_BRANCHFREE(branchFreemybrancingrule)
 {
    SCIP_BRANCHRULEDATA* branchruledata;
 ...
 static
 SCIP_DECL_BRANCHEXECLP(branchExeclpmybrancingrule)
 {  /*lint --e{715}*/
    SCIP_BRANCHRULEDATA* branchruledata;
    SCIP_VAR** lpcands;
    SCIP_VAR* bestcand;
    ...
    *result = SCIP_DIDNOTRUN;
    ...
    /* invalidate the current row mybrancingrule data which is initialized on the fly when looping over the candidates */
 ...
 /** event execution method of mybrancingrule branching which handles bound change events of variables */
 static
 SCIP_DECL_EVENTEXEC(eventExecmybrancingrule)
 {  /*lint --e{715}*/
    SCIP_BRANCHRULEDATA* branchruledata;
    SCIP_EVENTHDLRDATA* eventhdlrdata;
    SCIP_VAR* var;
 ...
 /** creates the mybrancingrule branching rule and includes it in SCIP */
 SCIP_RETCODE SCIPincludeBranchrulemybrancingrule(
    SCIP*                 scip                /**< SCIP data structure */
    )
 {
    ...
    /* create mybrancingrule branching rule data */
    branchruledata = NULL;
    ...
    SCIP_CALL( SCIPincludeEventhdlrBasic(scip, &branchruledata->eventhdlr, EVENTHDLR_NAME,
          "event handler for dynamic acitivity mybrancingrule updating",
          eventExecmybrancingrule, eventhdlrdata) );
    assert( branchruledata->eventhdlr != NULL);
    SCIP_CALL( SCIPsetEventhdlrFree(scip, branchruledata->eventhdlr, eventFreemybrancingrule) );
    ...
    assert(branchrule != NULL);
    SCIP_CALL( SCIPsetBranchruleCopy(scip, branchrule, branchCopymybrancingrule) );
    SCIP_CALL( SCIPsetBranchruleFree(scip, branchrule, branchFreemybrancingrule) );
    SCIP_CALL( SCIPsetBranchruleExitsol(scip, branchrule, branchExitsolmybrancingrule) );
    SCIP_CALL( SCIPsetBranchruleExecLp(scip, branchrule, branchExeclpmybrancingrule) );
 
    /* add mybrancingrule branching rule parameters */
